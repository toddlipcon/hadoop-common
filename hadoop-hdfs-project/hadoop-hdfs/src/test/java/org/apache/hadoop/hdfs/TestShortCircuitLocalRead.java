...
public class TestShortCircuitLocalRead {
  ...
  public void doTestShortCircuitRead(boolean ignoreChecksum, int size,
    ...
    conf.set(DFSConfigKeys.DFS_DATANODE_DOMAIN_SOCKET_PATH,
        new File(sockDir.getDir(),
          "TestShortCircuitLocalRead.%d.sock").getAbsolutePath());
    if (simulatedStorage) {
  ...
  public void testDeprecatedGetBlockLocalPathInfoRpc()
    ...
    try {
      ...
      try {
        ...
        Assert.fail("expected getBlockLocalPathInfo to throw " +
  ...
  public void testHandleTruncatedBlockFile() throws IOException {
    ...
    try {
      ...
      fsIn.readFully(buf, 0, buf.length);
      TestBlockReaderLocal.compareArrayRegion(original, 0, buf, 0,
          original.length);
      fsIn.close();
      fsIn = null;
    } finally {
